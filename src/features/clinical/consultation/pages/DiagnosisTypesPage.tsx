...
import type { DiagnosisType } from '../types/consultation.types';
...
export const diagnosisTypesApi = createCrudApi<DiagnosisType, FormValues>('/consultations/diagnosis-types', 'consultations.diagnosis-types');
const hooks = createCrudHooks('consultations.diagnosis-types', diagnosisTypesApi);
export const useDiagnosisTypes = hooks.useList;

function YesNo({ value }: { value: boolean }) {
  return <span>{value ? 'Yes' : 'No'}</span>;
}

const columns: ColumnsType<DiagnosisType> = [
...
const fields: FieldConfig<FormValues>[] = [
  { type: 'text', name: 'name', label: 'Name', required: true, placeholder: 'Dengue' },
...
export function DiagnosisTypesPage() {
  return (
    <CrudResourcePage<DiagnosisType, FormValues>
      title="Diagnosis Types"
      singularTitle="Diagnosis Type"
      screenKey="consultations.diagnosis-types"
      breadcrumbs={[{ label: 'Clinical' }, { label: 'Consultation' }, { label: 'Diagnosis Types' }]}
      columns={columns}
      fields={fields}
      schema={schema}
